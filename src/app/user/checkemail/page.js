...
export default function PreSignup(props) {
  ...
  const submitHandler = () => {
    ...
    Axios.post(getCustomer, data).then((response) =>
      response.data === "Exists" ? setError(true) : setError(false),
    );
    if (showError === true) {
      ...
      <div>
        <div className="flex flex-col justify-center">
          ...
          <div className="flex justify-center py-7">
            <p className="text-sm">
              Let's check if you're already registered with us
            </p>
          </div>
          <div className="relative flex justify-center py-7 pl-10">
            ...
            <input
              ...
              className="md:w-1/5 w-4/5 pl-5 text-black rounded-lg hover:shadow-white hover:shadow ring-0 border-0"
            />
          ...
          {showError && (
            <div className="flex justify-center py-7">
              <p className="bg-gray-500 px-3 rounded-lg py-1">
          ...
          {showError && (
            <div className="flex justify-center gap-5 py-7">
              <Link
                href="/user/signup"
                className="px-3 bg-gray-500 rounded-lg py-1"
              >
                Sign up with a new account
              </Link>
              <button className="px-3 bg-gray-500 rounded-lg py-1">
                Log in using exisiting account
              </button>
            </div>
          )}
        </div>
